<?php
class Distributionwrapup_model extends MY_Model {

	protected $_table = 'qm.dist_wrapup';
	protected $primary_key = 'dwid';
	...
	function get_dist($dist_id){
		...
		$this->db->join('callcenter.wrapups', 'qm.dist_wrapup.wrapupid = callcenter.wrapups.wrapupid');
	...
	function get_all_wrapup($tenant_id)
	{
		$this->load->database('callcenter', TRUE);
		$this->db->select('*');
		$this->db->from('callcenter.wrapups');
