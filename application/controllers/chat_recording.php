...
class Chat_recording extends CI_Controller {
	...
	function index()
	{
		...
		$data['main'] = 'recording/chat_index';
		$data['js_function'] = array('chat_recording');
	...
	function assign_form_agent()
	{
		...
		if($query2 = $this->user_model->sort_by_supervisor($userid)->get_all())
		{
		...
		$data['main'] = 'recording/assign_form_agent2';
		$data['js_function'] = array('chat_recording');
		$this->load->view('template/template',$data);
	}
	...
	function submit_form()
	{
		...
		//$query = $this->chatrecording_model->get($record_id);

		//$userid = $query->userid;

		// update userid in recording records

		$this->recqm_model->insert($data);
		$this->chatrecording_model->update($record_id,array('status'=>'3','userid'=>$userid,'tenantid'=>$tenant_id,'supervisor_id'=>$supervisor_id));
	...
	function search_recording()
	{
		...
		$config['base_url'] = site_url().'/chat_recording/search_recording';
		...
		$config['total_rows'] = count($this->chatrecording_model->search_list($tenant_id, $userid, $search_param, null, null)->get_all());
		$content_data['total_rows'] = count($this->chatrecording_model->search_list($tenant_id, $userid, $search_param, null, null)->get_all());

		$this->pagination->initialize($config);
		// end Pagination

		if($query = $this->chatrecording_model->search_list($tenant_id, $userid, $search_param, $config['per_page'], $this->uri->segment(3))->get_all())
		{
			$content_data['data_records'] = $query;
		}
		$this->load->model('chatmessage_model');
		if($query = $this->chatmessage_model->get_all())
		{
			$content_data['get_chat_records'] = $query;
		}
		$content_data['page'] = 'chat_recordings2';
		$content_data['main'] = 'recording/chat_index';
		$content_data['js_function'] = array('chat_recording');
